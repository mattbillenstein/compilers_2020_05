...
class WabbitWasmModule:
    def __init__(self):
        self.mod = WasmModule('heeeeelllllpppp')
        self._print = WasmImportedFunction(self.mod, 'runtime','_print', [i32], [])
        # self.main = WasmFunction(self.mod, 'main', [], [i32])
        self.main = WasmFunction(self.mod, 'main', [], [])

    def do_print(self):
        print('doing print')
        self.main.call(self._print)
...
@generate.register(PrintStatement)
def _(node, env):
    #put what you wanna print on the stack
    generate(node.child, env)
    #call print
    env.do_print()

@generate.register(Integer)
def _(node, env):
    env.main.iconst(node.value)
...
def write_out(model):
    ...
    # m.main.iconst(39)
    # m.do_print()
    m = generate_program(model)
    with open('out_runtime.wasm', 'wb') as file:
        file.write(encode_module(m.mod))
